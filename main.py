...
with st.sidebar:
    # Title
    st.title("Toshiro Frederick E-Commerce Dashboard")
...
ax[0].set_title("Most Popular Products", loc="center", fontsize=50)
...
ax[1].set_title("Least Popular Products", loc="center", fontsize=50)
ax[1].tick_params(axis='y', labelsize=35)
ax[1].tick_params(axis='x', labelsize=30)

st.pyplot(fig)

# Rating by customers for service
st.subheader("Rating by customers for service")
col1,col2 = st.columns(2)

with col1:
    avg_review_score = review_score.mean()
    st.markdown(f"Average Rating from customers: **{avg_review_score}**")

with col2:
    most_common_review_score = review_score.value_counts().index[0]
    st.markdown(f"Most rating score count: **{most_common_review_score}**")

fig, ax = plt.subplots(figsize=(12, 6))
sns.barplot(x=review_score.index, 
            y=review_score.values, 
            order=review_score.index,
            palette=["#6A9AB0" if score == common_score else "#B7B7B7" for score in review_score.index]
            )
...
with tab1:
    customers_city = all_df['customer_city'].value_counts().head(10)

    # Streamlit App
    st.subheader("Customers Count from Each City")
...
with tab2:
    ...
    sns.barplot(x=order_status.index,
                ...
                palette=["#6A9AB0" if score == common_status else "#B7B7B7" for score in order_status.index]
                )
...
with tab3:
    ...
    with st.expander("See Explanation"):
        st.write('Depends on the map, the red dot represents the customer location, and the customers is spread across Brazil. The most common customer location is in the state of Sao Paulo.')

st.caption('Copyright (C) Toshiro Frederick 2024')